...
from classes.SensorGrid import SensorGrid
                ...
                if response[0]:
                    with self.lock:
                        if not self.timing_instance.timestamp1:
                            self.lights.handle_lights([GPIO.LOW, GPIO.HIGH, GPIO.HIGH, GPIO.LOW])
                            dt1 = datetime.now()
                ...
                if response[1]:
                    with self.lock:
                        if not self.timing_instance.timestamp2 and self.timing_instance.timestamp1:
                            self.lights.handle_lights([GPIO.LOW, GPIO.LOW, GPIO.HIGH, GPIO.LOW])
                            dt2 = datetime.now()
